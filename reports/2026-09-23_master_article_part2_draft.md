...
- сузили задачу по слову человека с видео до **одной неподвижной картинки**.
...
Критерий приёмки поставил человек, и он главнее любой метрики: **результат
засчитывается только клипом против сырого видео корпуса.** Мыло на первой
версии допустимо, если появляются сцены и новый розыгрыш даёт новое осмысленное
видео. Как этот критерий появился, рассказано в § 5: до него мы четыре раза
подряд верили судье, который ошибался.
...
**17.1b — поправка человека:** «датасет на 76 % процедурный, а задача —
видеогенератор». Приор переучен только на сценах, в компактном временном
базисе: DCT, 16 коэффициентов из 40, каждый z-нормирован (92 288 чисел).
...
Человек сформулировал её так, чтобы её нельзя было потом подменить: пока нельзя
вбить сид в noise2state и получить видео, как на настоящем клипе, результата
нет. Формулировка и всё исключённое записаны в
[отдельном документе](2026-09-20_the_seed_problem.md).
...
## 9. Остановка перед обучением: аудит метрик

Человек остановил работу: «прежде чем продолжать обучение, стоит пересмотреть
валидацию… ворота я один раз уже поймал на лжи». Аудит нашёл, что именно
сломано. [Отчёт](2026-09-21_the_draw_distribution.md).
...
Отображение близко к тождественному. Отсюда наблюдение человека, что
...
латентом, а с тестовым (ISS-0010); из-за этого я сообщал перелёт радиуса 44 %
...
Разбиение прошло через ошибку, пойманную человеком на проектировании: первая
версия назначала колонки ближайшему центру, патчи выходили от 1 до 5 колонок,
и эквивариантность, ради которой всё строилось, ломалась.
...
плеч над радиусом разыгрываемого шума, делегировано подагенту, $0. Два плеча
...
Человек сузил задачу: сцена не видео, а одна неподвижная картинка, движение
пока неважно. И объект — **721 × 2**, а не 721 × 32: «в чём тогда смысл
выкидывания времени».
[Отчёт 29](2026-09-21_step29_a_scene_as_one_picture.md).

**Есть ли в этом пространстве неподвижная картинка вообще.** Первый тест был
поставлен неверно: я обнулил 15 из 16 временных коэффициентов настоящего
состояния и подал в 13B состояние, которого не вызывал ни один клип. Человек
назвал тест невалидным, вывод снят. Верная форма — менять стимул: один кадр,
удержанный всё окно, проходит `стимул → мозг → состояние → 13B` с **r 0,953**,
лучше, чем движущийся клип (0,916), и отрисовка действительно неподвижна.
Довод «T4/T5 — детекторы движения, значит статики нет» ложен.
...
**Вердикт человека** — из двух частей, и обе в силе. Сначала: глазами это уже
ближе к сцене, чем всё, что было до. Потом, после отрисовки срезов: пока всё,
что он видит, проходит через мыльный отрисовщик, он не может сказать, хороший
поток или плохой. Связывающая — вторая.
...
- **Невалидный тест статики** (23.1): поймал человек, вывод снят.
- **Неравные патчи** (23): поймал человек на проектировании.
- **Отображение съедало контраст** (ISS-0012): сетки картинок нормировали каждую
  ...
  подписи сравнивали. Поймал человек, глядя на клип, который помнил
  чёрно-белым.
- **Проверка чисел в отчётах** (2026-09-22): четыре агента пересчитали каждое
  число трёх отчётов из артефактов. Девять расхождений, среди них «lr поднят
  впятеро» вместо вдесятеро, число без всякого артефакта (15,8 % вместо 16,3 %)
  и два перебора в пользу результата. Все исправлены с пометкой.
...
результата — контроль из того же прогона, а последнее слово — за глазом
человека на сыром видео.
...
Следующий порядок, принятый человеком: перемерить кодом из репозитория два
...
- Цитаты человека в тексте смягчены до смысла, где оригинал резок.

...
def run(model: str, prior_ckpt, gen_ckpt, manifest: dict, columns: dict, procedural_file: Path, *,
    ...
    log(f"{n_samples} states from the prior in {time.time() - t0:.0f} s")
...
def main(argv=None) -> int:
    ...
    r = run(a.model, Path(a.prior), Path(a.gen), manifest, columns, proc, n_samples=a.samples, n_clips=a.clips,
            ...
            seed=a.seed)
    (out / "samples17_local.json").write_text(json.dumps(r["summary"], indent=1), encoding="utf-8")
    np.savez_compressed(out / "samples17_local.npz", **r["arrays"])
    gt = r["summary"]["gates"]
